...
static void USART_INIT_FN(uint32_t nspeed, uint32_t nbits, uint32_t nevent, uint32_t nstop)
{
	...
	// ---------------- DMA 相关配置 ----------------
		...
		while(LL_DMA_IsEnabledStream(DMAx,DMA_TX_STRMx));
...
void USART_IRQ_HANDLER(void)
{
	...
	if (LL_USART_IsActiveFlag_IDLE(USARTx)) { //空闲中断
		...
		#else 	
			unsigned short pkt_len = TTYSx.rxmax - LL_DMA_GetDataLength(DMAx,DMA_RX_STRMx);//得到当前包的长度
		#endif 
