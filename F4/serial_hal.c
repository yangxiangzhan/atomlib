...
#define DMA_TX_IRQ_HANDLER      DMAx_Streamy_IRQHandler(xDMA,xDMATxStream) //中断函数名
...
#define DMA_RX_IRQ_HANDLER      DMAx_Streamy_IRQHandler(xDMA,xDMARxStream) //中断函数名
...
static void usart_dma_init(void)
{
	...
	LL_DMA_DisableStream(DMAx,DMA_TX_STREAM);//发送暂不使能
	LL_DMA_DisableStream(DMAx,DMA_RX_STREAM);//发送暂不使能
...
void USART_IRQ_HANDLER(void) 
{
	...
	pkt_len = serial_rx.pktmax - LL_DMA_GetDataLength(DMAx,DMA_RX_STREAM);//得到当前包的长度
